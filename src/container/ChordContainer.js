...
export default class ChordContainer extends Component {
  ...
  updateHighlight = () => {
    ...
    if(!ObjectUtils.checkAllNotNull(accidental, inversion, letter, octave, quality)){
      return;
    }
  ...
  handleQuality = (quality) => {
    this.setState({ quality: Chord.Quality[quality] }, () => this.updateHighlight());
  }
  ...
  render = () => {
    return (
      <Segment attached='bottom'>
        ...
        <ButtonGroup source={Object.keys(Chord.Quality).slice(0, 8)} onClick={this.handleQuality} toString={i => StringUtils.toCamelCase(i.replace(/_/g, ' '))} />
        <Header as='h2' dividing>Inversion</Header>
        <ButtonGroup source={Chord.Inversion.values()} onClick={this.handleInversion} toString={i => StringUtils.toCamelCase(i.toString())} />
        {
